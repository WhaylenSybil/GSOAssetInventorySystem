...
if (isset($_POST['btn_WMR_save'])) {

    // Check if the uploaded file is not empty and there are no errors
    if (isset($_FILES['scanndPRS']) && $_FILES['scanndPRS']['error'] == 0) {
        $targetDirectory = './PRS Scans/';
        $targetFile = $targetDirectory . basename($_FILES['scanndPRS']['name']);
        ...
        if (file_exists($targetFile)) {
            ...
            if (move_uploaded_file($_FILES['scanndPRS']['tmp_name'], $targetFile)) {
                echo "The scanned document has been uploaded successfully.";
                // Store the file path in the database
                $scanndPRS = $targetFile;
            } else {
    ...
    if ($row['ID'] == 0) {
        ...
        $pre_stmt->bind_param('ssssssssssssssssssssssssss',
            $type,
            $dateReturnedRecorded,
            $ItemNo,
            $prsNumber,
        ...
        if ($pre_stmt->execute()) {
            ...
            if ($modeOfDisposalStmt->execute()) {
                ...
                if ($updatesOrCurrentStatusStmt->execute()) {
                    ...
                    $log_stmt->execute();
                    $log_stmt->close();
                    echo '<script type="text/javascript">alert("Added a WMR"); window.location = "WMR.php";</script>';
                } else {
