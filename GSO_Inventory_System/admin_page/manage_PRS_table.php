...
while ($rows = mysqli_fetch_array($result)) {
    $formattedDate = (!empty($rows["dateReturnedRecorded"]) && $rows["dateReturnedRecorded"] != "0000-00-00") ? date("m-d-Y", strtotime($rows["dateReturnedRecorded"])) : "";
    ...
    $scannedPRSPath = $rows["scannedPRS"]; // Path to the scanned ARE document

        // Conditionally create the "View Scanned Supporting document" link
        if (!empty($scannedPRSPath)) {
            $scannedPRSLink = '<a href="' . $scannedPRSPath . '" target="_blank">View Scanned Supporting Document</a>';
        } else {
    ...
    <tr>
      ...
      <td><?php echo $rows["withAttachment"]; ?></td>
      <td><?php echo $rows["withCoverPage"]; ?></td>
