...
if (isset($_POST['btn_ARE_save'])) {
    ...
    if (isset($_FILES['scanned_docs']) && $_FILES['scanned_docs']['error'] == 0) {
        $targetDirectory = './ARE Scans/';
        $targetFile = $targetDirectory . basename($_FILES['scanned_docs']['name']);
        ...
        if (file_exists($targetFile)) {
            ...
            if (move_uploaded_file($_FILES['scanned_docs']['tmp_name'], $targetFile)) {
    ...
    if ($row['ID'] == 0) {
        ...
        if ($pre_stmt->execute()) {
            ...
            function displayModalWithRedirect($message, $redirectPage) {
                ...
                echo '<button class="ok-button" onclick="redirectToPage(\'active_PPE.php\')">OK</button>';
                echo '</div>';
                echo '</div>';
            }

            // Show a modal dialog with the message and redirect to "PRS.php"
            displayModalWithRedirect("Added an ARE Property", "active_PPE.php");
        } else {
            // Error inserting data
        ...
        displayModalWithRedirect("ARE property already exists", "add_ARE_Registy.php");
    }
