...
<body>
    ...
    // Check if a responsibility center parameter is provided in the URL
    if (isset($_GET['center'])) {
        $responsibilityCenter = urldecode($_GET['center']);
        ...
        $sql = "SELECT
                ...
                WHERE
                    i.responsibilitycenter_id = ?
                ORDER BY i.responsibilitycenter_id DESC";
        $stmt = $connect->prepare($sql);

        if ($stmt) {
            $stmt->bind_param("s", $responsibilityCenter);
            $stmt->execute();
            $result = $stmt->get_result();
            
            // Fetch and display the ARE properties
            if ($result->num_rows > 0) {
                echo "<h3 style='text-align: center; line-height: 2em;'>Report on Physical Count of Property, Plant, and Equipment<br><span style='text-decoration: underline;'>$responsibilityCenter</span><br> (Type of Property, Plant, and Equipment)</h3>";
